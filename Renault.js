...
async function createWidget(data) {
    ...
    if (typeof (data.batteryStatus) != 'undefined') {
        ...
        if (data.batteryStatus.attributes.plugStatus == 1 && data.batteryStatus.attributes.chargingStatus == "0") {
            plugStateLabel = "➤ Starting charge"
    ...
    if (typeof (data.batteryStatus) != 'undefined') {
        ...
        const batteryStatusLabel = BatteryStack.addText("Charge level")
    ...
    if (typeof (data.batteryStatus) != 'undefined') {
        ...
        const range = Math.floor(data.batteryStatus.attributes.batteryAutonomy * 0.621371).toString()
        const RangeStatusVal = RangeStack.addText(range + " mi")
    ...
    if (ZOE_Phase == 2 && typeof (data.batteryStatus) != 'undefined') {
        if (typeof (data.batteryStatus.attributes.batteryAvailableEnergy) != 'undefined') {
            ...
            const AvEnergyStatusLabel = AvEnergyStack.addText("Charge level")
    ...
    if (typeof (data.cockpitStatus) != 'undefined') {
        ...
        const MileageStatusLabel = MileageStack.addText("Milage")
        MileageStatusLabel.font = Font.mediumSystemFont(12)
        let mileage = Math.round(data.cockpitStatus.attributes.totalMileage * 0.621371).toLocaleString()
        const MileageStatusVal = MileageStack.addText(mileage.toString() + " mi")
    ...
    if (typeof (data.locationStatus) != 'undefined') {
        ...
        if (mapProvider == "google") {
            ...
            LocationVal.url = "http://maps.apple.com/?q=My+Car&ll=" + data.locationStatus.attributes.gpsLatitude + "," + data.locationStatus.attributes.gpsLongitude
        }
    ...
    const AcLabel = AcStack.addText("Pre heat.")
...
async function getStatus(endpoint, version = 1, kamareonURL, account_id, VIN, gigyaJWTToken, kamareonAPI) {
    ...
    url = kamareonURL + '/commerce/v1/accounts/' + account_id + '/kamereon/kca/car-adapter/v' + version + '/cars/' + VIN + '/' + endpoint + '?country=UK'
    req = new Request(url)
    req.method = "GET"
...
async function postStatus(endpoint, jsondata, version, kamareonURL, account_id, VIN, gigyaJWTToken, kamareonAPI) {
    url = kamareonURL + '/commerce/v1/accounts/' + account_id + '/kamereon/kca/car-adapter/v' + version + '/cars/' + VIN + '/actions/' + endpoint + '?country=UK'
